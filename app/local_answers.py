...
def find_local_answer(question: str) -> dict | None:
    ...
    price_words = {"quanto custa", "preco", "valor", "quanto pago", "pagar"}
    deadline_words = {"prazo", "quanto tempo", "demora", "duracao", "avaliar"}

    if "diagnostico" in q or ("avaliar" in q and "computador" in q):
        parts, evidence = [], []
        asks_repair = contains_any(q, {"aprovar", "aprovado", "reparo", "conserto"})
        asks_price = contains_any(q, price_words) or asks_repair
        asks_deadline = contains_any(q, deadline_words)
